...
<body class="grain-overlay relative overflow-x-hidden bg-slate-50">
    ...
    @if (isset($header))
        <div class="relative z-10 mx-auto max-w-7xl px-4 pt-4 sm:px-6 lg:px-8">
            <div class="rounded-3xl border border-white/70 bg-white/55 px-6 py-6 shadow-sm backdrop-blur-sm">
                {{ $header }}
            </div>
        </div>
    @endif

    <main id="contenido" class="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {{ $slot }}
    </main>
